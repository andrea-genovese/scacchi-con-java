...
/**
 * Questa classe rappresenta un pezzo generico non istanziabile su una
 * {@code Scacchiera}
 *
 */
public abstract class Pezzo {
	...
	/**
	 * Aggiunge la mossa alla lista se la casa di destinazione esiste e non ha sopra
	 * un pezzo amico
	 * 
	...
	protected ArrayList<Mossa> getMosseAlfiere() {
		...
		int xStart = x - 1;
		int yStart = y - 1;
		while (Scacchiera.isValidSquare(xStart, yStart) && s.getArray()[yStart][xStart] == null) { // verso
																									// alto-sinistra
	...
	public Pezzo clone(Scacchiera nuova) {
		if (this instanceof Re) {
			...
			throw new IllegalCallerException("this non � istanza di nessun pezzo");

	}

	public static Pezzo createFromChar(int x, int y, char c, Scacchiera s) {
		boolean isWhite = c < 'a';
		switch (c) {
			case 'p', 'P':
				return new Pedone(x, y, isWhite, s);
			case 'n', 'N':
				return new Cavallo(x, y, isWhite, s);
			case 'b', 'B':
				return new Alfiere(x, y, isWhite, s);
			case 'r', 'R':
				return new Torre(x, y, isWhite, s);
			case 'q', 'Q':
				return new Donna(x, y, isWhite, s);
			case 'k','K': return new Re(x, y, isWhite, s);
			default: throw new IllegalArgumentException("'"+c+"'"+"non è un carattere valido");
		}
	}
